...
@section('content')
									...
									<div class="main-post">
											<div class="blog-post-inner">
													<div class="post-info">
															<div class="left-area">
																<a class="avatar" href="#">
																	<img src="{{ asset('uploads/user/'.$post->user->image) }}" alt="Profile Image">
																</a>
															</div>
													...
													<ul class="tags">
														@foreach ($post->categories as $row)
															<li><a href="{{ route('post.by.postByCategory',$row->slug) }}">{{ $row->name }}</a></li>
														@endforeach
													</ul>
											</div><!-- blog-post-inner -->
							...
							<div class="col-lg-4 col-md-12 no-left-padding">
									<div class="single-post info-area">
											<div class="sidebar-area about-area">
													<h4 class="title"><b>ABOUT {{ Str::upper($post->user->name)  }}</b></h4>
											...
											<div class="tag-area">
													...
													<ul>
														@foreach ($post->tags as $row)
															<li><a href="{{ route('post.by.postByTag',$row->slug) }}">{{ $row->name }}</a></li>
														@endforeach
													</ul>
	...
	<div id="myCarousel" class="carousel slide" data-ride="carousel">
			...
			<ul class="carousel-indicators">
				@foreach ($post->galleries as $key => $row)
					<li data-target="#myCarousel" data-slide-to="{{$key}}" class="" id="myCarousel-{{$key}}"></li>
				@endforeach
			</ul>
			<!-- The slideshow -->
			<div class="carousel-inner">
				@foreach ($post->galleries as $key => $row)
					<div class="carousel-item" id="carousel-item-{{$key}}">
						<img src="{{ asset('uploads/post/galleries/'.$row->gallery_image) }}" alt="Los Angeles" width="1100" height="320">
					</div>
				@endforeach
			</div>
	...
	<section class="recomended-area section">
		<div class="container">
			<div class="row">
				@foreach ($randomposts as $row)
					<div class="col-lg-4 col-md-6">
							<div class="card h-100">
									<div class="single-post post-style-1">
											<div class="blog-image"><img src="{{ asset('uploads/post/'.$row->image) }}" alt="Blog Image"></div>
											<a class="avatar" href="#"><img src="{{ asset('uploads/user/'.$row->user->image) }}" alt="Profile Image"></a>
											<div class="blog-info">
													<h4 class="title">
...
@push('js')
	...
	{!! Toastr::message() !!}
	<script>
		$('#myCarousel-0').addClass('active');
		$('#carousel-item-0').addClass('active');
	</script>
    {{-- <script>
			$(document).ready(function() {
					var owl = $(".owl-carousel").owlCarousel({items:1, center:true, loop:false, dots:false,
					margin:10, callbacks:true, URLhashListener:true,autoplayHoverPause:true, startPosition:'URLHash'});
