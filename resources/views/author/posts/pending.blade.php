		...
		<div class="col-md-12">
			<div class="box">
				<div class="box-header">
					...
					<span><a href="{{ route('author.posts.create') }}" class="btn btn-primary btn-sm pull-right">Create Post</a></span>
        </div>
						...
						<tbody>
							@foreach ($posts as $key => $row)
								<tr>
									...
									<td>
										<a href="{{ route('author.posts.show',$row->id) }}" class="btn btn-info btn-xs"><i class="fa fa-eye"></i></a>
										<a href="{{ route('author.posts.edit',$row->id) }}" class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a>
										<a href="javascript:deleteObject({{ $row->id }})" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></a>
										<form id="frmDeletePost-{{ $row->id }}" style="display: none" action="{{ route('author.posts.destroy',$row->id) }}" role="form" method="POST" enctype="multipart/form-data">
											{{ csrf_field() }}
											{{ method_field('DELETE') }}
										</form>
									</td>
